...
namespace ServerCore
{
	...
	public:
		...
		struct  promise_type
		{
			static void* operator new(const std::size_t size) { return Mgr(MemoryMgr)->Allocate(size); }
			static void operator delete(void* const pointer, const std::size_t size) { Mgr(MemoryMgr)->Release(pointer); }
